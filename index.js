...
var ProtWeb3Provider = function (config) {
...
ProtWeb3Provider.prototype.isConnected = function () {
    return true;
}

ProtWeb3Provider.prototype.send = function (payload) {
    var response = {
        jsonrpc: "2.0",
        id: payload.id
    };
    ...
    switch (payload.method) {
        ...
        case "eth_uninstallFilter":
            ...
            break;
        default:
            throw new Error("Prot does not support calling " + payload.method + " synchronously without a callback. Please provide a callback parameter to call " + payload.method + " asynchronously.");
    }
}


ProtWeb3Provider.prototype.sendAsync = function(payload,callback){
...
ProtWeb3Provider.prototype._sendAsync = function(payload){
...
ProtWeb3Provider.prototype.eth_accounts = function () {
    return this.address ? [this.address] : [];
}

ProtWeb3Provider.prototype.eth_coinbase = function() {
    return this.address;
}

ProtWeb3Provider.prototype.net_version = function(){
    return this.chainId.toString(10) || null;
}

ProtWeb3Provider.prototype.eth_sign = function(payload) {
    this.postMessage("signMassage",payload.id,{data: payload.params[1]});
}

ProtWeb3Provider.prototype.personal_sign = function(payload){
    this.postMessage("signPersonalMessage",payload.id,{data:payload.params[0]});
}

ProtWeb3Provider.prototype.eth_signTypedData = function(payload){
    this.postMessage("signTypedMessage",payload.id,{data: payload.params[0]});
}

ProtWeb3Provider.prototype.eth_sendTransaction = function(payload){
    this.postMessage("signTransaction",payload.id,{data: payload.params[0]});
}


ProtWeb3Provider.prototype.eth_newFilter = function(payload){
...
ProtWeb3Provider.prototype.eth_newBlockFilter = function(payload) {
    this.filterMgr.newBlockFilter()
        .then(filterId => this.sendResponse(payload.id,filterId))
        .catch(error => this.sendError(payload.id,error));
}

ProtWeb3Provider.prototype.eth_newPendingTransactionFilter = function(payload){
    this.filterMgr.newPendingTransactionFilter()
        .then(filterId => this.sendResponse(payload.id,filterId))
        .catch(error => this.sendError(payload.id,error));
}

ProtWeb3Provider.prototype.eth_uninstallFilter = function(payload) {
    this.filterMgr.uninstallFilter(payload.params[0])
        .then(filterId => this.sendResponse(payload.id,filterId))
        .catch(error => this.sendError(payload.id,error));
}

ProtWeb3Provider.prototype.eth_getFilterChanges = function(payload){
    this.filterMgr.getFilterChanges(payload.params[0])
        .then(data => this.sendResponse(payload.id,filterId))
        .catch(error => this.sendError(payload.id,error));
}

ProtWeb3Provider.prototype.getFilterLogs = function(payload) {
    this.filterMgr.getFilterLogs(payload.params[0])
        .then(data => this.sendResponse(payload.id,data))
        .catch(error => this.sendError(payload.id,error));
}

ProtWeb3Provider.prototype.postMessage = function(handler,id,data){
...
ProtWeb3Provider.prototype.sendResponse = function(id,result){
...
ProtWeb3Provider.prototype.sendError = function(id,error) {
...
module.exports = ProtWeb3Provider